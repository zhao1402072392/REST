...
def add_sparse_args(parser):
    ...
    parser.add_argument('--prun_high', action='store_true', help='prun high weights.')
...
class Masking(object):
    ...
    def init(self, mode='ERK', density=0.05, erk_power_scale=1.0, train_loader=None):
        ...
        if mode == 'global_magnitude':
            ...
            layer_wise_sparsities = SNIP(self.module, self.density, train_loader, self.device)
            # print  ("layer_wise_sparsities",layer_wise_sparsities)
            # re-sample mask positions
            for sparsity_, name in zip(layer_wise_sparsities, self.masks):
                
                self.masks[name][:] = sparsity_
            ...
            if self.dataset == "cmnist":
                num_classes = 10
            if self.dataset=="bffhq":
                num_classes=2

            print ("num_classes",num_classes)
            layer_wise_sparsities = GraSP(self.module, self.density, train_loader, self.device,num_classes=num_classes)
            # re-sample mask positions
            # print  ("layer_wise_sparsities",layer_wise_sparsities)
            for sparsity_, name in zip(layer_wise_sparsities, self.masks):
                self.masks[name][:] = sparsity_
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name not in self.masks: continue
                print(name, 'sparsity:', (weight!=0).sum().item()/weight.numel())
    ...
    def add_module(self, module, density, sparse_init='ER', train_loader=None):
        self.module=module
        ...
        self.init(mode=sparse_init, density=density, train_loader=train_loader)
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                if self.death_mode == 'magnitude':
                    new_mask = self.magnitude_death(mask, weight, name, self.args.prun_high)
    ...
    def magnitude_death(self, mask, weight, name, prun_high):
    ...
    def print_density(self):
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                ...
                # sum 
                dense_size+=num_nonzeros
                total_size+=num_all

                val = '{0}: {1}, density: {2:.3f}'.format(name,  num_nonzeros, num_nonzeros/float(num_all))
                print(val)

        print('Total Model parameters  density:', dense_size/ float(total_size) )
