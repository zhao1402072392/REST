...
def train(model, criterion, dataset,
    ...
    mask = None
    if args.sparse:
        decay = CosineDecay(args.death_rate, len(dataset['train_loader']) * args.n_epochs)
        ...
        mask.add_module(model, sparse_init=args.sparse_init, density=args.density, train_loader=dataset['train_loader'])
    ...
    for epoch in range(epoch_offset, epoch_offset+args.n_epochs):
            ...
            if curr_val_acc > best_val_acc:
                ...
                if args.sparse:
                    if args.sparse_init == 'snip':
                        best_model_name = str(epoch) + 'snip_best_model.pth'
                    elif args.sparse_init == 'GraSP':
                        best_model_name = str(epoch) + 'grasp_best_model.pth'
                    elif args.sparse_init == 'ERK' and args.death == 'SET':
                        best_model_name = str(epoch) + 'set_best_model.pth'
                    elif args.sparse_init == 'ERK' and args.death == 'magnitude':
                        best_model_name = str(epoch) + 'rigl_best_model.pth'
                else:
                    best_model_name = str(epoch) + 'dense_best_model.pth'
                torch.save(model, os.path.join(args.log_dir, best_model_name))
                logger.write(f'Best model saved at epoch {epoch}\n')


        # infe using best val model
        if args.robust or args.reweight_groups:
            if min(val_loss_computer.avg_group_acc) != 0:
                curr_val_acc = min(val_loss_computer.avg_group_acc)
            else:
                curr_val_acc = val_loss_computer.avg_acc
        else:
        ...
        if curr_val_acc > best_val_acc:
            ...
            print ("now test acc",min(test_loss_computer.avg_group_acc) )
            best_model_dic=copy.deepcopy(model.state_dict())

        # print("val_loss_computer.avg_group_acc",val_loss_computer.avg_group_acc, val_loss_computer.avg_acc)
