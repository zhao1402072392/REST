...
def main():
    ...
    if resume:
        model_1 = torch.load(os.path.join(args.log_dir, 'last_model.pth'))
        d = train_data.input_size()[0]
    elif model_attributes[args.model]['feature_type'] in ('precomputed', 'raw_flattened'):
        assert pretrained
        # Load precomputed features
        d = train_data.input_size()[0]
        model_1 = nn.Linear(d, n_classes)
        model_1.has_aux_logits = False
    elif args.model == 'resnet50':
        model_1 = torchvision.models.resnet50(pretrained=pretrained)
        d = model_1.fc.in_features
        model_1.fc = nn.Linear(d, n_classes)
    elif args.model == 'resnet34':
        model_1 = torchvision.models.resnet34(pretrained=pretrained)
        d = model_1.fc.in_features
        model_1.fc = nn.Linear(d, n_classes)
    elif args.model == 'wideresnet50':
        model_1 = torchvision.models.wide_resnet50_2(pretrained=pretrained)
        d = model_1.fc.in_features
        model_1.fc = nn.Linear(d, n_classes)
    elif args.model =='resnet50vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_1 = resnet50vw(args.resnet_width, num_classes=n_classes)
    elif args.model =='resnet18vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_1 = resnet18vw(args.resnet_width, num_classes=n_classes)
    elif args.model =='resnet10vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_1 = resnet10vw(args.resnet_width, num_classes=n_classes)
    ...
    pretrained = not args.train_from_scratch
    if resume:
        model_2 = torch.load(os.path.join(args.log_dir, 'last_model.pth'))
        d = train_data.input_size()[0]
    elif model_attributes[args.model]['feature_type'] in ('precomputed', 'raw_flattened'):
        assert pretrained
        # Load precomputed features
        d = train_data.input_size()[0]
        model_2 = nn.Linear(d, n_classes)
        model_2.has_aux_logits = False
    elif args.model == 'resnet50':
        model_2 = torchvision.models.resnet50(pretrained=pretrained)
        d = model_2.fc.in_features
        model_2.fc = nn.Linear(d, n_classes)
    elif args.model == 'resnet34':
        model_2 = torchvision.models.resnet34(pretrained=pretrained)
        d = model_2.fc.in_features
        model_2.fc = nn.Linear(d, n_classes)
    elif args.model == 'wideresnet50':
        model_2 = torchvision.models.wide_resnet50_2(pretrained=pretrained)
        d = model_2.fc.in_features
        model_2.fc = nn.Linear(d, n_classes)
    elif args.model =='resnet50vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_2 = resnet50vw(args.resnet_width, num_classes=n_classes)
    elif args.model =='resnet18vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_2 = resnet18vw(args.resnet_width, num_classes=n_classes)
    elif args.model =='resnet10vw':
        assert not pretrained
        assert args.resnet_width is not None
        model_2 = resnet10vw(args.resnet_width, num_classes=n_classes)
    elif args.model == 'bert':
        assert args.dataset == 'MultiNLI'
    ...
    train_moe(model_1,model_2, criterion, data, logger, train_csv_logger, val_csv_logger, test_csv_logger, args, epoch_offset=epoch_offset)
