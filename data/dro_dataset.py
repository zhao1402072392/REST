...
class DRODataset(Dataset):
    ...
    def get_loader(self, train, reweight_groups, **kwargs):
        if not train: # Validation or testing
            ...
            # print('weights', weights, weights.size()) #weights tensor([ 1.0524,  1.0524,  1.0524,  ..., 20.0990, 20.0990, 20.0990]) torch.Size([45062])
            # print('self._group_array',self._group_array, self._group_array.size()) #self._group_array tensor([1, 1, 1,  ..., 0, 0, 0]) torch.Size([45062])

            weights_tmp = self._group_array.clone()  # 先克隆一个 x 的副本，以免修改原始 tensor x

            # 使用逻辑运算符和条件语句，将 x 中的 0 替换为 80
            weights_tmp[self._group_array == 0] = 80
            weights = weights_tmp
            # print('weights', weights, weights.size())
            # print('self._group_array',self._group_array, self._group_array.size())
